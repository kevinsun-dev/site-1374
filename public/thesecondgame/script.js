      ...
      if (playerData['phoneNumber'].endsWith(getCookie("phoneNumber"))) {
        ...
        const setCurrentRoom = () => {
          if (board.currentTile == board.start) {
            ...
            switch (rText) {
              case 0: displayText.innerText = `The door closes behind you with a sharp click. You step over the number ${board.currentTile + board.room_num_add} as you reach into your folder to find the next clue.
            
              Available Doors: ${board.map[board.currentTile].exits.map(x => x + board.room_num_add).join(", ")}, and Back`;
                break;
              case 1: displayText.innerText = `You arrive in the next room, ${board.currentTile + board.room_num_add} it seems. You look around at the doors and walls, vibrant hues dulled by the dim light. You fumble through your folder for a new clue.
            
              Available Doors: ${board.map[board.currentTile].exits.map(x => x + board.room_num_add).join(", ")}, and Back`;
                break;
              case 2: displayText.innerText = `The faint light of a keypad guides your way and illuminates room ${board.currentTile + board.room_num_add}. The folder feels heavy in your hand as you open it once more.
        ...
        const chooseRoom = () => {
          ...
          switch (rText) {
            ...
            case 0: displayText.innerText = `"Please, please work" you whisper under your breath. To your delight, the little LCD panel blinks twice, then the beautiful words "Access Granted" appear. These words then disappear as it prompts you to type in a particular door to unlock.
              
              Type in the room number you'd like to access.
              
              Available Doors: ${board.map[board.currentTile].exits.map(x => x + board.room_num_add)}`;
              break;
            case 1: displayText.innerText = `With hesitant confidence, you type in the unlock code. The brief delay feels like eons. Suddenly, a happy chirp emanates from the device, and a new prompt appears.
              
              Type in the room number you'd like to access.
              
              Available Doors: ${board.map[board.currentTile].exits.map(x => x + board.room_num_add)}`;
              break;
            case 2: displayText.innerText = `You take a deep breath. The last key is pressed. The display flashes, and you wait for the alarms to sound. They don't, and the screen flickers as it offers a new prompt.
